...
async def archive(request, sleep_duration, photos_dir, logger):
    ...
    try:
        while not process.stdout.at_eof():
            archive_data = await process.stdout.read(512000)
...
def main():
    ...
    archive_handler = partial(archive, sleep_duration=args.sleep, photos_dir=args.path, logger=logger)
